...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command([f"{LEECH@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_message_handler)
    #
#    incoming_gdrive_message_handler = MessageHandler(
#        incoming_gdrive_message_f,
#        filters=Filters.command([f"{GLEECH@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
#    )
...
#    app.add_handler(incoming_size_checker_handler)
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=Filters.command([f"{CLEAR@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_g_clear_handler)
    #    
    incoming_youtube_dl_handler = MessageHandler(
        incoming_youtube_dl_f,
        filters=Filters.command([f"{YTDL@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_dl_handler)
    #
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command([f"{STATUS@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(status_message_handler)
    #
    cancel_message_handler = MessageHandler(
        cancel_message_f,
        filters=Filters.command([f"{CANCEL@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    eval_message_handler = MessageHandler(
        eval_message_f,
        filters=Filters.command(["eval@Fasterleechbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command([f"{SAVE@Fasterleechbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
